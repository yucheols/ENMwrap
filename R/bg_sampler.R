...
bg_sampler <- function(envs, n, occs_list = NULL, buffer_list = NULL, bias.grid = NULL, excludep = T, method = 'random') {
  ...
  if (method == 'random') {
    ...
    for (i in 1:length(buffer_list)) {
      mask.envs <- raster::mask(envs, buffer_list[[i]])
      bg <- dismo::randomPoints(mask = mask.envs, n = n, p = occs_list[[i]], excludep = excludep) %>% as.data.frame()
      colnames(bg) = c('long', 'lat')
    ...
    bg <- raster::xyFromCell(object = bias.grid,
                             sample(which(!is.na(values(raster::subset(x = raster::raster(envs[[1]]), 1)))),
                                    size = n, prob = values(x = bias.grid)[!is.na(values(raster::subset(x = raster::raster(envs[[1]]), 1)))])) %>% as.data.frame()
    colnames(bg) = c('long', 'lat')
    bg.out <- bg
  }
  return(bg.out)
